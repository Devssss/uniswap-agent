export async function GET() {
  const manifest = {
    "accountAssociation": {
      "header": "eyJmaWQiOjkxNTIsInR5cGUiOiJjdXN0b2R5Iiwia2V5IjoiMHgwMmVmNzkwRGQ3OTkzQTM1ZkQ4NDdDMDUzRURkQUU5NDBEMDU1NTk2In0",
      "payload": "eyJkb21haW4iOiJhcHAuZXhhbXBsZS5jb20ifQ",
      "signature": ""
    },
    "miniapp": {
      "version": "1",
      "name": "Sentinel AI",
      "homeUrl": process.env.APP_URL || "https://ais-dev-2ynqwk7hl4umnf2zqb5crg-615601803900.asia-southeast1.run.app",
      "iconUrl": "https://picsum.photos/seed/sentinel/200/200",
      "splashImageUrl": "https://picsum.photos/seed/sentinel-splash/1200/600",
      "splashBackgroundColor": "#050507",
      "webhookUrl": `${process.env.APP_URL || "https://ais-dev-2ynqwk7hl4umnf2zqb5crg-615601803900.asia-southeast1.run.app"}/api/webhook`,
      "subtitle": "ETH DEX & Liquidity Aggregator",
      "description": "AI-powered scanner for Uniswap liquidity and DEX opportunities.",
      "screenshotUrls": [
        "https://picsum.photos/seed/s1/1024/768",
        "https://picsum.photos/seed/s2/1024/768"
      ],
      "primaryCategory": "finance",
      "tags": ["defi", "dex", "ai", "base", "uniswap"],
      "heroImageUrl": "https://picsum.photos/seed/sentinel-hero/1200/600",
      "tagline": "Real-time AI Liquidity Analysis",
      "ogTitle": "Sentinel AI",
      "ogDescription": "Aggregating ETH DEX data in real-time.",
      "ogImageUrl": "https://picsum.photos/seed/sentinel-og/1200/600",
      "noindex": true
    }
  };
  return Response.json(manifest);
}
